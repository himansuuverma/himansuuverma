...
const SkillCard = ({ name, icon }) => (
  <motion.div
    ...
    className="flex flex-col items-center p-4 bg-primary-bg rounded-lg shadow-md hover:shadow-accent-1/10 transition-shadow duration-300"
    whileHover={{ y: -5 }}
  >
  ...
  return (
    <section id="skills" className="py-20 bg-secondary-bg">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <SectionTitle id="skills-title">Skills & Tech</SectionTitle>
            ...
            <div className="flex flex-wrap justify-center sm:justify-start gap-3">
              {skills.coreCompetencies.map((competency, index) => (
                <motion.span
                  key={index}
                  variants={skillItemVariants}
                  className="bg-primary-bg text-text-primary py-2 px-4 rounded-full text-sm font-mono shadow-sm"
                >
                  {competency}
                </motion.span>
              ))}
