          ...
          */}
          ...
          {leadershipAndInvolvement.map((item, index) => (
            ...
            >
              ...
              <div className="flex-shrink-0 w-8 h-8 sm:w-10 sm:h-10 bg-primary-bg rounded-full border-2 border-accent-1 flex items-center justify-center mr-4 sm:mr-6 relative z-10">
              ...
              <div className="flex-grow p-4 sm:p-6 bg-secondary-bg rounded-lg shadow-xl hover:shadow-accent-1/20 transition-shadow duration-300">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-1 sm:mb-2">
                    ...
                    <p className="text-xs sm:text-sm text-text-primary/80 font-mono mt-1 sm:mt-0">{item.duration}</p>
                </div>
                <div className='flex gap-5 my-3'>
                <p className="text-accent-1 text-3xl mb-2">{item.icon}</p>

                <p className="text-md sm:text-md font-semibold mb-2 sm:mb-3">{item.organization}</p>
                </div>
                ...
                {item.points.length > 0 ? (
                  ...
                  <p className="text-text-primary/80 italic text-xs sm:text-sm">Details forthcoming.</p>
                )}
              </div>
            </motion.div>
          ))}
